function HypC = HypC_ProdIntP(zp,awzp,GP,dp,ngl,N,nPan)
% HypC = HypC_ProdIntP(zp,awzp,GP,dp,ngl,N,nPan)
...
% Uses panelwise product integration in parameter variable
    ...
    for i = 1:nPan
        ...
        for j = 1:3
            ...
            wHcmp = wHinitP(ptgtr,psctr,zp(scInd),zp(tgInd),awzp(scInd),j,ngl,cc);
            HypC(tgInd,scInd) = wHcmp;
        end
    end
end

function wHcmp = wHinitP(ptgtr,psctr,zpsc,zptg,awzpsc,j,ngl,cc)
%Calculates local compensation term wHcmp
    ...
    wHcmpTemp = -log(abs(psctr.'-ptgtrc));
    if j == 2
        wHcmpTemp(1:ngl+1:ngl^2) = log(abs(cc*zptg));
    end
    ...
    V=psctr.^(0:ngl-1);
    wHcmp = zeros(ngl);
    wHcmp(closetg,:) = cc*(Q/V).*abs(zpsc).'+awzpsc.'.*wHcmpTemp;
end